...
#tests
echo "Starting tests"
echo "test 1: register zone"

result=$(runCommand "add nice")
expected="GeographicZone : {name=nice}"
assertEquals "$expected" "$result"
globalResult+=($?)

echo "test 2: register zone the same"

result=$(runCommand "add nice")
expected="403 : {\"error\":\"Geographic zone nice already exists\",\"details\":null}"
assertEquals "$expected" "$result"
globalResult+=($?)
